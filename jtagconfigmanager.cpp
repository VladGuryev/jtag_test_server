 ...
 * метод отправляет запрос (его нужно сформировать) для того чтобы плис вернула ответ,
 * вроде того что в такой то ячейке лежит некоторое число
 * @param
 */
...
QString JtagConfigManager::IntToHexConverter(QString data)
{
    ...
    // приходится конвертировать из int представления в строку hex символов
    int count = data.size();
    QString convertedData;

    for(int i = 0; i < 16 - count; i++){   //дополняем нулями недостающие позиции в строке из 16 символов
